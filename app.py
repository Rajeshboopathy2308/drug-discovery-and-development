...
file_path = 'D:\\drug_model\\dataset\\dataset.csv'
        ...
        if compound1 and compound2:
            ...
            try:
                ...
                if predicted_drug != "invalid":
                    ...
                    flag = 0
                    return render_template('index_1.html', prediction=predicted_drug, description=description)
                elif flag:
                    predicted_drug =get_drug_description(f'''Will these two compounds react? {compound1} and {compound2}''')
                    main_list=[]
                    list_desc=[]
                    desc=''
                    for i in predicted_drug:
                        if i!='*':
                            desc+=i
                        elif i=='*':
                            list_desc.append(desc)
                    main_list.append(list_desc)
                    if desc:
                        list_desc.append(desc)
                        main_list.append(list_desc)
                    '''predict =fI am studying potential drug interactions. For the following two compounds, typically found in medications, predict the outcome of their combination:{compound1} and {compound2}.Specifically, tell me:
                    ...
                    return render_template('index_1.html', prediction=main_list)
...
def predict():
        ...
        respone = get_smile_description(smiles_input)
        ...
        prompt=f"Using your chemical knowledge base, please identify the chemical compound corresponding to the following SMILES notation: {smiles_input}."
        res=get_drug_description(prompt)
        report = f"{res} has {''.join(map(str,pIC50))}M againt the {selected_option} disease"
...
def get_smile_description(smile):
    ...
    return MolWt,MolLogP,NumHAcceptors,NumHDonors
    
@app.route('/backmodel2page')
def backmodel2page():
